...
{
    $req_prod="SELECT *FROM produit_vente INNER JOIN categories on categories.id_cat=produit_vente.id_cat where vendu_prodV is null";
}
else{
        $req_prod="SELECT *FROM produit_vente INNER JOIN categories on categories.id_cat=produit_vente.id_cat where valid_prodV=1 and vendu_prodV is null ";
}
