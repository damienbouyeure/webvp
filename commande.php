...
<div class="container">
	...
	<div class="col-xs-2">
     ...
     $req_livr="SELECT * FROM livraison where nom_livraison='$nom_livraison'";
     $res_livr=mysqli_query($bdd,$req_livr);
     $row_livr=mysqli_fetch_assoc($res_livr);
     $_SESSION['id_livraison']=$row_livr['id_livraison'];
...
$ligne=mysqli_fetch_assoc($res);         	
					...
					<div class="row">
						<div class="col-xs-1"></div>
						<div class="col-xs-4">
							<h4 class="product-name"><strong>Livraison</strong></h4><h4><small><?php echo $row_livr["nom_livraison"]; ?></small></h4>
						</div>
												<div class="col-xs-1"></div>
						<div class="col-xs-6">
							<div class="col-xs-6 text-right">
								<h6><strong><?php echo $row_livr["payement_livraison"]; ?><span class="text-muted">€</span></strong></h6>
							</div>
         ...
         $res=mysqli_query($bdd, $req);
         $ligne=mysqli_fetch_assoc($res);
  $_SESSION['prix_total']=$ligne["total_prix_panier"]+$row_livr['payement_livraison'];
        echo $_SESSION['prix_total']; ?> € </strong></h4>
						</div>
						<div class="col-xs-3">
							<a type="button" class="btn btn-success btn-block" href="valider_commande.php">
								Commander
							</a>
						</div>
					</div>
